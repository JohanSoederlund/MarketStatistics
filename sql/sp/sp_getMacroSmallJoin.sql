...
BEGIN
	SELECT DISTINCT 
		...
		SUM(c2.fatalities) AS 'sum fatalities', 
        Cast(ROUND(m.population/1000) as char) 'population / T'
    
	FROM Conflict c2
		inner join  Macro m
			ON m.country_id = c2.country_id and m.macro_year = c2.conflict_year
		inner join  Country c
			ON c.country_id = c2.country_id AND IF(p_country = '*', 
            c.country_name LIKE '%', c.country_name = p_country)
